...
@router.post("/create")
async def user_create(new_user: UserCreateSchema, session: AsyncSession = Depends(get_async_session)):
...
async def user_edit(user_id: int, new_user: UserEditSchema, session: AsyncSession = Depends(get_async_session)):
    # Check if the email is already registered
    db_email_query = select(User).where(User.email == new_user.email)
    db_email = await session.scalar(db_email_query)
    if db_email:
